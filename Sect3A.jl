...
function bc_upd(ϵ::Float64, ev_start::Bool, n_agents::Int64, averaging)
    bc_ar = ev_start == true ? collect(range(1/n_agents, stop=1, length=n_agents)) : rand(n_agents)
    ...
    @views while true
        for i in 1:n_agents
            fa = findall(@. abs(bc_ar[:, j] - bc_ar[i, j]) <= eps)
...
@everywhere function radicalized_normals_count(ϵ::Float64, ev_start::Bool, r_opinion::Float64, n_normals::Int64, n_radicals::Int64, averaging)
    st = ev_start == true ? collect(range(1/n_normals, stop=1, length=n_normals)) : rand(n_normals)
    δ = ev_start == true ? 10^-12 : 0.0
    eps = ϵ + δ
    rd = fill(r_opinion, n_radicals)
...
@everywhere function radicalized_normals_average(ϵ::Float64, r_opinion::Float64, n_normals::Int64, n_radicals::Int64, averaging)
    st = rand(n_normals)
    rd = fill(r_opinion, n_radicals)
    bc_ar = vcat(rd, st)
    v = vcat(rd, st)
...
@everywhere function sim_mixed(eps::Float64, r_opinion::Float64, n_indifs::Int64, n_radicals::Int64, α::Float64, τ::Float64)
...
Gadfly.spy(truth_res[end:-1:1, :],
    ...
    Guide.xlabel("Radicals"), Guide.ylabel("Free riders"),
    Guide.yticks(ticks=ticks), Scale.y_continuous(labels=ylabelname),
