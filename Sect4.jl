...
function mixed_community(ϵ::Float64, ev_start::Bool, r_opinion::Float64, n_truthseekers::Int64, n_indifs::Int64, n_radicals::Int64, α::Float64, τ::Float64, averaging)
    n_normals = n_truthseekers + n_indifs
    st = ev_start == true ? collect(range(1/n_normals, stop=1, length=n_normals)) : rand(n_normals)
    ...
    @views while true
        for i in 1:n_normals
            fa = findall(@. abs(bc_ar[:, j] - bc_ar[i, j]) <= eps + δ)
            @inbounds v[i] = i <= n_truthseekers ? α*averaging(bc_ar[fa, j]) + (1 - α)*τ : averaging(bc_ar[fa, j])
        end
        bc_ar = hcat(bc_ar, v)
    ...
    bb[1:n_truthseekers] .= "normal"
    bb[n_truthseekers + 1:n_normals] .= "free rider"
    bb[n_normals + 1:n_normals + n_radicals] .= "radical"
...
plot(out_mixed, x=:steps, y=:value, group=:variable, color=:Agent, Geom.point, Geom.line,
    ...
    Scale.color_discrete_hue(gen_colors, levels = ["normal", "free rider", "radical"], preserve_order=true),
...
function bc_upd_rad_with_cd(ϵ_start::Float64, ev_start::Bool, r_opinion::Float64, n_normals::Int64, n_radicals::Int64, averaging_op, averaging_eps)
    st = ev_start == true ? collect(range(1/n_normals, stop=1, length=n_normals)) : rand(n_normals)
    δ = ev_start == true ? 10^-12 : 0.0
    rd = fill(r_opinion, n_radicals)
    bc_ar = vcat(rd, st)
    ...
    @views while true
        for i in (n_radicals + 1):(n_radicals + n_normals)
            fa = findall(@. abs(bc_ar[:, j] - bc_ar[i, j]) <= eps[i] + δ)
...
@everywhere function radicalized_normals_count_with_cd(ϵ_start::Float64, ev_start::Bool, r_opinion::Float64, n_normals::Int64, n_radicals::Int64, averaging_op, averaging_eps)
    st = ev_start == true ? collect(range(1/n_normals, stop=1, length=n_normals)) : rand(n_normals)
    δ = ev_start == true ? 10^-12 : 0.0
    rd = fill(r_opinion, n_radicals)
    bc_ar = vcat(rd, st)
...
function radical_count_parallel_with_cd(ev_start::Bool, r_opinion::Float64, n_normals::Int64, averaging_op, averaging_eps)
    res = Array{Int64,2}(undef, 50, 50)
    for j in 1:50
        @inbounds res[:, j] = pmap(i -> radicalized_normals_count_with_cd(i/100, ev_start, r_opinion, n_normals, j, averaging_op, averaging_eps), 1:50)
    end
    return res
end
...
Gadfly.spy(rotl90(out_cd),
    ...
    Guide.xlabel("ϵ"), Guide.ylabel("Radicals"),
...
@everywhere function radicalized_average_cd(ϵ_start::Float64, r_opinion::Float64, n_normals::Int64, n_radicals::Int64, averaging_op, averaging_eps)
    ...
    @views while true
        @inbounds for i in (n_radicals + 1):(n_radicals + n_normals)
            fa = findall(@. abs(bc_ar[:, j] - bc_ar[i, j]) <= eps[i])
            v[i] = averaging_op(bc_ar[fa, j])
            tmp[i - n_radicals] = averaging_eps(eps[fa])
        end
        eps = vcat(eps_rad, tmp)
...
function mixed_community_cd(ϵ_start::Float64, ev_start::Bool, r_opinion::Float64, n_truthseekers::Int64, n_indifs::Int64, n_radicals::Int64, α::Float64, τ::Float64, averaging_op, averaging_eps)
    ...
    @views while true
        for i in 1:n_normals
            fa = findall(@. abs(bc_ar[:, j] - bc_ar[i, j]) <= eps[i] + δ)
            @inbounds v[i] = i <= n_truthseekers ? α*averaging_op(bc_ar[fa, j]) + (1 - α)*τ : averaging_op(bc_ar[fa, j])
            tmp[i] = averaging_eps(eps[fa])
        end
...
function cost_sim_ri_cd(ϵ_start::Float64, α::Float64, τ::Float64, r_opinion::Float64, n_truthseekers::Int64, n_indifs::Int64, n_radicals::Int64, averaging, n_sim::Int64)
...
data = vcat(without, with10, with25, with50)
groups = categorical(repeat(1:4, inner=nsim))
df_aov = DataFrame(data = data, groups = groups)
model = fit(LinearModel, @formula(data ~ groups), df_aov, contrasts = Dict(:groups => EffectsCoding()))
anova(model)
